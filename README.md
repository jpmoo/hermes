...
- `POST /api/notes/hover-insight` — body `{ noteId }`. Tag suggestions combine: **neighbor** (Ollama, `from_vocab` true only if tag is approved on **parent, siblings, or immediate children**), **new** (Ollama, up to **6** `from_vocab` false hyphenated tags), **connected** (approved tags on **linked** peers not on the hovered note). Context always includes the hovered note (even empty). No vector similar-note list. Response: `tagSuggestions[]`, `similarNotes` (`[]`), `persistedLinks`. **Ollama** required for model tags.
