...
- `GET /api/user/settings`, `PATCH /api/user/settings` — `GET` returns `{ noteTypeColors, similarNotesMinChars: number | null, similarNotesMinDefault, similarNotesLimitResultsToMinChars: boolean, noteHistory }`. `PATCH` accepts `noteTypeColors`, `similarNotesMinChars` (integer **0–500** — minimum trimmed length for the hovered note before similar-notes run, or **`null`** to drop the override and use `similarNotesMinDefault` from env), `similarNotesLimitResultsToMinChars` (**boolean** or **`null`** to clear stored value; default **false** — when **true** and minimum **> 0**, similar-note candidates must meet the same length), and `noteHistory` (up to 20 recent `{ noteId, title, threadPath, threadRootId?, visitedAt }`). (JWT). Run `psql "$DATABASE_URL" -f server/src/db/migrations/008_user_settings_json.sql` if upgrading an existing DB.
...
- **`POST /api/notes/:id/attachments`** — **multipart/form-data**, field name **`files`** (one or more parts, up to 20 per request). Same upload shape as the web Stream UI. **`Authorization: Bearer <jwt>`** required. Use this for **large images/PDFs**; no base64, streams straight into Postgres. Max size per file: `HERMES_MAX_ATTACHMENT_BYTES` (default 20MB). **OCR + LLM note fill** (PDF/image when the note body is still empty) runs only if you send **`X-Hermes-Attachment-Ocr: 1`**. The Stream web UI does **not** send this—attachments are stored only. MCP (`hermes_attach_files`) and `curl` add the header when you want OCR on the JWT attachments route.
