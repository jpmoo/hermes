...
- `GET /api/notes/search-content?q=...&limit=...` — substring search in full note text (no Ollama). Optional **`firstLine=1`**: match only the **first line** — it must **start with** `q` (case-insensitive) **or** contain a **space +** `q` as a word prefix (for titles like `Meeting Charlie` when typing `@Ch`). Used for @-mention pickers; keyword Search view keeps the default full-body substring behavior.
